...
for i = 1: length(Start)
    currInterval = (binStartEffort(i):minutes(1):binEndEffort(i))';
    ...
    if (binStartEffort(i)-Start(i)) ~= 0
       bin(1) =  (minutes(1)-(Start(i)-binStartEffort(i)))/minutes(1);
       sec(1) = seconds(minutes(1)-(Start(i)-binStartEffort(i)));
    end
    
    if End(i) > binEndEffort(i)
       bin(end) = (End(i) - binEndEffort(i))/minutes(1);
