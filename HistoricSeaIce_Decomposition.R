...
#Linear Model
lm(formula = concentration~Yr,data = SeaIceMed)
yrmedian<-ggplot(SeaIceMed, aes(x=Yr, y=concentration))+
  geom_point()
yrmedian = yrmedian + geom_smooth(method="lm", col="black")
yrmedian = yrmedian + stat_regline_equation(label.x = 1850, label.y = 80) + theme_bw() +
  labs(title = "Yearly Median Sea Ice Concentration",
       x = "Year",
       y = "Median Sea Ice Concentration (0 to 100)")
yrmedian
